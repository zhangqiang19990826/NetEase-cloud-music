		...
		<div class="Mymv">
			...
			<div class="portrait">
				<div class="headPortrait">
					<img :src="SingerData.avatarUrl" />
				</div>
				
				<p>{{SingerData.nickname}}</p>
			</div>
			<!-- 歌曲名 -->
			...
			<!-- 发布时间 -->
			<div class="Mvtime">
				<p><span>发布:</span><span>{{MvData.publishTime | formatTime}}</span> </p>
				<p style="margin-left: 20px;"><span>播放:</span><span>{{MvData.playTime | formatCount}}</span> </p>
			</div>
...
<script>
	export default{
		...
		methods:{
			...
			// 获取MV数据
			async getMvData(){
				const {data:res} = await this.$http.get('/video/detail?id='+this.ID)
				console.log(res)
				if(res.code== 200){
					this.MvData = res.data
					this.SingerData = res.data.creator
			...
			// 获取专辑评论
			getComment(){
				this.$http.get('/comment/video',{params:{id:this.ID,limit:50,offset:(this.playListCurrentPage-1)*50}}).then(res =>{
					console.log(res)
